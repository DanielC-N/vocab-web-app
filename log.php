...
<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mode']) && $_POST['mode'] === 'oui') {
        
        if (!empty($id)) {
            $doesExist = insertWordLog($mot_fr, $mot_en, $note, $id);
            if ($doesExist) {
                header("Location: index.php"); // Redirection vers index.php
                exit;
            } else {
                echo "Le mot existe déjà dans la base de données.";
            }
        } else {
            var_dump($_POST);
